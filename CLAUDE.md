...
- **laravel-auth** — Token-based authentication with `DynamicGuard`, `AuthenticateToken` action, `TokenType` enum. Provides `auth:dynamic` guard used in API routes.
- **laravel-support** — Eloquent concerns (`HasExpiration`, `HasSlugs`, `HasChildren/HasParent`, `InMemory`), custom validation rules (`ExistsEloquent`, `UniqueEloquent`), password reset utilities.
- **laravel-essentials** — Middleware (`UseRequestId`, `UseHeaderGuards`), database commands (`db:make`, `db:drop`), `Overseer` request inspection, `DogmaManager` for code principles enforcement, `EssentialsConfig`.

### Application Structure

- **Controllers** — `app/Controllers/` (no `Http` subdirectory)
- **Models** — `app/Models/` with `Builders/`, `Concerns/`, `Scopes/` subdirectories
- **Services** — `app/Services/` for business logic (e.g., `TokenService`, `UserService`)
- **Resources** — `app/Resources/` for JSON:API-style API responses
- **Route Registration** — `app/ApiRouteRegistrar.php` invokable class pattern, registered in `AppServiceProvider`
...
- Migration filenames use numeric prefixes (e.g., `1000000001_create_users_table.php`)

### API Routes

All under `/api` prefix. Protected routes use `auth:dynamic` middleware. Resources: users, user tokens.
