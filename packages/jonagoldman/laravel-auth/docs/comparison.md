...
**No longer a regression.** laravel-auth now supports both modes via alternative traits:
...
- **Opt-in polymorphic:** `HasMorphTokens` trait with `MorphMany`/`MorphOne` — swap one trait on the owner model, override `owner()` on the token model to return `MorphTo`, and publish the polymorphic migration via `laravel-auth-morph-migrations`.
