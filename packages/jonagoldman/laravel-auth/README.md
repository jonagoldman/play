...
```bash
composer require jonagoldman/laravel-auth
```
...
use JonaGoldman\Auth\Concerns\IsAuthToken as IsAuthTokenConcern;
...
use JonaGoldman\Auth\Concerns\HasTokens as HasTokensConcern;
...
| `authenticate_session` | `JonaGoldman\Auth\Middlewares\AuthenticateSession` |
...
```bash
php artisan vendor:publish --tag=laravel-auth-morph-migrations
```
...
```php
use JonaGoldman\Auth\Concerns\HasMorphTokens as HasMorphTokensConcern;
use JonaGoldman\Auth\Contracts\HasMorphTokens;
...
| `JonaGoldman\Auth\Events\TokenAuthenticated` | After successful bearer auth | `token: Model&IsAuthToken` |
